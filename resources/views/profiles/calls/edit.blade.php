        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    Edit Call Log - {{ $profile->full_name }}
                </div>

                <div class="card-body">
                    <form action="{{ route('profiles.calls.update', ['profile' => $profile, 'call' => $call]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('call_type') is-invalid @enderror" id="call_type" name="call_type" required>
                                @foreach($callTypes as $value => $label)
                                    <option value="{{ $value }}" {{ old('call_type', $call->call_type) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('call_status') is-invalid @enderror" id="call_status" name="call_status" required>
                                @foreach($statuses as $value => $label)
                                    <option value="{{ $value }}" {{ old('call_status', $call->call_status) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('remarks') is-invalid @enderror" 
                                      id="remarks" name="remarks" rows="3">{{ old('remarks', $call->remarks) }}</textarea>
                            @error('remarks')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" 
                                   ...
                                   value="{{ old('followup_date', $call->followup_date ? $call->followup_date->format('Y-m-d\TH:i') : '') }}">
                            @error('followup_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
