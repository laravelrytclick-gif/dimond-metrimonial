        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    Add Family Member for {{ $profile->full_name }}
                </div>

                <div class="card-body">
                    <form action="{{ route('profiles.family.store', $profile) }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('member_type') is-invalid @enderror" id="member_type" name="member_type" required>
                                ...
                                @foreach($memberTypes as $value => $label)
                                    <option value="{{ $value }}" {{ old('member_type') == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('age') is-invalid @enderror" 
                                           id="age" name="age" value="{{ old('age') }}" min="0" max="150">
                                    @error('age')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="living_with_candidate" 
                                   name="living_with_candidate" value="1" {{ old('living_with_candidate') ? 'checked' : '' }}>
